...
// Forward wheel events from parent page into Unity iframe
(function() {
    ...
    document.addEventListener('wheel', function(e) {
        ...
        if (iframe) {
            ...
            iframe.focus();
            try {
                iframe.contentWindow.postMessage({ type: 'wheel', deltaY: e.deltaY }, '*');
            } catch(_) {}
        }
    }, { passive: false });
})();
...
<div class="layout">
  ...
  <aside class="right-panel">
    ...
    container.addEventListener('click', e => {
        const li = e.target.closest('[data-subcomp]');
        if (!li)
            return;
...
function mediaBlock(url, caption, cameraOrbit) {
    if (!url) {
...
function updateRight(app, detail) {
    const right = app.querySelector('[data-right="sistem"]');
    if (!right)
        return;
    right.innerHTML = `
